...
void CnmemSpace::updateSpace(CnmemSpace::Op op, size_t size) {
  if (op == ADD)
    ...
    if (required_space > free_bytes) this->out_of_memory = true;
    free_bytes -= required_space;
  }
}

bool CnmemSpace::isAvailable() { return !out_of_memory; }

size_t CnmemSpace::getConsumed() { return (initial_free_bytes - free_bytes); }

void CnmemSpace::updateMaxConsume(size_t &max_consume) {
  max_consume = max_consume > (initial_free_bytes - free_bytes) ? max_consume
