...
void NeuralNet::getLoss(void *X, int *y, double learning_rate, std::vector<float> &fwd_dnn_lag,
  ...
  // cudaMemGetInfo(&free_bytes, &total_bytes);
  // int aft0 = free_bytes;
  // logfile << "Allocated to layer 0: " << (bef0 - aft0)<< " free: " << free_bytes / (1024.0 *
  // 1024.0 * 1024.0) << "\n";
  cudaMemcpy(layer_input[0], X, batch_size * input_channels * input_h * input_w * data_type_size,
             cudaMemcpyHostToDevice);
  if (train == true) cudaMemcpy(this->y, y, batch_size * data_type_size, cudaMemcpyHostToDevice);
  ...
  for (int i = 0; i < num_layers; i++) {
    ...
    if (layer_type[i] == CONV) {
      ...
      checkCUDNN(cudnnConvolutionForward(
          ...
          cur_params->W, cur_params->conv_desc, cur_params->fwd_algo, this->workspace,
          this->workspace_size, &beta, cur_params->output_tensor, layer_input[i + 1]));
  ...
  if (overhead != NULL) *overhead += temp_milli2;
  /***************************************************************************/
  ...
  *scalar_loss = computeLoss();  // Loss Computation
  ...
  for (int i = num_layers - 1; i >= 0; i--) {
    if (i > 0) {
      if (layer_type[i] == ACTV || layer_type[i] == SOFTMAX) dlayer_input[i] = dlayer_input[i + 1];
