                                   ...
                                   std::vector<float> &bwd_computation_time)
{
  ...
  // data_type_size, NULL)); checkCudaErrors(cudaMemcpy(layer_input[0], X,
  // batch_size * input_channels * input_h * input_w * data_type_size,
  // cudaMemcpyHostToDevice)); checkCudaErrors(cudaMemcpy(this->y, y, batch_size
  ...
  // * data_type_size, NULL)); space_tracker.updateSpace(CnmemSpace::SUB,
  ...
  // *)params[num_layers - 1]; 	if (data_type == CUDNN_DATA_FLOAT) {
  // 		checkCudaErrors(cudaMemset(dlayer_input[num_layers], 0, batch_size *
  // num_classes *
  // sizeof(float))); 		softmaxLossBackProp<float><<<ceil(1.0 * batch_size /
  // BW), BW>>>(this->y, (float *)layer_input[num_layers], (float
  // *)dlayer_input[num_layers], batch_size, num_classes, softmax_eps);
  // 	}
  // 	else if (data_type == CUDNN_DATA_DOUBLE) {
  // 		checkCudaErrors(cudaMemset(dlayer_input[num_layers], 0, batch_size *
  // num_classes *
  // sizeof(double))); 		softmaxLossBackProp<double><<<ceil(1.0 * batch_size
  // / BW), BW>>>(this->y, (double *)layer_input[num_layers], (double
  // *)dlayer_input[num_layers], batch_size, num_classes, softmax_eps);
  // 	}
  // }
