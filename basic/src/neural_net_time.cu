...
void NeuralNet::getComputationTime(void *X, int *y, double learning_rate,
  ...
  // checkCudaErrors(cudaMemcpy(layer_input[0], X, batch_size * input_channels * input_h * input_w *
  // data_type_size, cudaMemcpyHostToDevice)); checkCudaErrors(cudaMemcpy(this->y, y, batch_size *
  // data_type_size, cudaMemcpyHostToDevice));
  ...
  for (int i = 0; i < num_layers; i++) {
    ...
    if (layer_type[i] == CONV) {
      ...
      if (data_type == CUDNN_DATA_FLOAT) {
        checkCUBLAS(cublasSgemm(cublas_handle, CUBLAS_OP_N, CUBLAS_OP_N, cur_params->C_out,
        ...
        checkCUBLAS(cublasDgemm(cublas_handle, CUBLAS_OP_N, CUBLAS_OP_N, cur_params->C_out,
  ...
  // ---------------------- vDNN start ----------------------
  // checkCNMEM(cnmemMalloc(&dlayer_input[num_layers], batch_size * num_classes * data_type_size,
  // NULL)); space_tracker.updateSpace(CnmemSpace::SUB, layer_input_size[num_layers] *
  // data_type_size);
  ...
  // 	if (data_type == CUDNN_DATA_FLOAT) {
  // 		checkCudaErrors(cudaMemset(dlayer_input[num_layers], 0, batch_size * num_classes *
  // sizeof(float))); 		softmaxLossBackProp<float><<<ceil(1.0 * batch_size / BW),
  // BW>>>(this->y, (float *)layer_input[num_layers],
  // (float *)dlayer_input[num_layers], batch_size, num_classes, softmax_eps);
  // 	}
  // 	else if (data_type == CUDNN_DATA_DOUBLE) {
  // 		checkCudaErrors(cudaMemset(dlayer_input[num_layers], 0, batch_size * num_classes *
  // sizeof(double))); 		softmaxLossBackProp<double><<<ceil(1.0 * batch_size / BW),
  ...
  for (int i = num_layers - 1; i >= 0; i--) {
    ...
    if (layer_type[i] == CONV) {
      ...
      if (data_type == CUDNN_DATA_FLOAT) {
        // bias backward
        checkCUBLAS(cublasSgemm(cublas_handle, CUBLAS_OP_N, CUBLAS_OP_N, cur_params->C_out, 1,
        ...
        // weight backward
        checkCUBLAS(cublasSgemm(cublas_handle, CUBLAS_OP_N, CUBLAS_OP_T, cur_params->C_out,
        ...
        // data backward
        if (i > 0)
          checkCUBLAS(cublasSgemm(cublas_handle, CUBLAS_OP_T, CUBLAS_OP_N, cur_params->C_in,
        ...
        // bias backward
        checkCUBLAS(cublasDgemm(cublas_handle, CUBLAS_OP_N, CUBLAS_OP_N, cur_params->C_out, 1,
        ...
        // weight backward
        checkCUBLAS(cublasDgemm(cublas_handle, CUBLAS_OP_N, CUBLAS_OP_T, cur_params->C_out,
        ...
        // data backward
        if (i > 0)
          checkCUBLAS(cublasDgemm(cublas_handle, CUBLAS_OP_T, CUBLAS_OP_N, cur_params->C_in,
    ...
    checkCNMEM(cnmemFree(layer_input[i], NULL));
    if (i > 0 && layer_type[i] != SOFTMAX) checkCNMEM(cnmemFree(dlayer_input[i], NULL));
  }
}
...
void NeuralNet::getTransferTime(void *X, int *y, double learning_rate,
  ...
  for (int i = 0; i < num_layers; i++) {
    ...
    checkCudaErrors(cudaMemcpyAsync(host_data, device_data, layer_input_size[i] * data_type_size,
    ...
    checkCudaErrors(cudaMemcpyAsync(device_data, host_data, layer_input_size[i] * data_type_size,
