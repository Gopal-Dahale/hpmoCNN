...
void DropoutLayerParams::initializeValues(cudnnHandle_t cudnn_handle,
                                          DropoutDescriptor *user_params, cudnnDataType_t data_type,
                                          int batch_size, cudnnTensorFormat_t tensor_format,
...
void DropoutLayerParams::allocateSpace(size_t &free_bytes, cudnnHandle_t cudnn_handle,
                                       DropoutDescriptor *user_params, long long seed) {
  cudaMalloc(&state, state_size);
  cudaMalloc(&reserved_space, reserved_space_size);
  checkCUDNN(cudnnSetDropoutDescriptor(dropout_desc, cudnn_handle, user_params->dropout_value,
                                       state, state_size, seed));
...
void BatchNormLayerParams::initializeValues(BatchNormDescriptor *user_params,
  ...
  if (user_params->mode == BATCHNORM_PER_ACTIVATION) {
    ...
    checkCUDNN(cudnnSetTensor4dDescriptor(sbmv_desc, tensor_format, data_type, 1,
                                          user_params->channels, user_params->h, user_params->w));
    sbmv_size = c * h * w;
  } else if (user_params->mode == BATCHNORM_SPATIAL) {
    mode = CUDNN_BATCHNORM_SPATIAL;
    checkCUDNN(cudnnSetTensor4dDescriptor(sbmv_desc, tensor_format, data_type, 1,
                                          user_params->channels, 1, 1));
    sbmv_size = c;
  }
...
void BatchNormLayerParams::allocateSpace(cudnnDataType_t data_type, size_t data_type_size,
                                         size_t &free_bytes) {
  ...
  if (data_type == CUDNN_DATA_FLOAT) {
    ...
    fillValue<double>
        <<<ceil(1.0 * allocation_size / BW), BW>>>((double *)scale, allocation_size, 1);
...
void BatchNormLayerParams::stepParams(cublasHandle_t cublas_handle, double learning_rate) {
  ...
  if (update_rule == SGD) {
    if (data_type == CUDNN_DATA_FLOAT) {
      cublasSaxpy(cublas_handle, sbmv_size, &Salpha, (float *)dscale, 1, (float *)scale, 1);
      cublasSaxpy(cublas_handle, sbmv_size, &Salpha, (float *)dbias, 1, (float *)bias, 1);
    } else if (data_type == CUDNN_DATA_DOUBLE) {
      cublasDaxpy(cublas_handle, sbmv_size, &Dalpha, (double *)dscale, 1, (double *)scale, 1);
      cublasDaxpy(cublas_handle, sbmv_size, &Dalpha, (double *)dbias, 1, (double *)bias, 1);
    }
  }
}
