...
class Solver {
 public:
  ...
  void getComputationTime(long num_epoch, std::vector<std::vector<float> > &fwd_computation_time,
                          std::vector<std::vector<float> > &bwd_computation_time);
  void stepComputationTime(int start_X, int start_y, std::vector<float> &fwd_computation_time,
                           std::vector<float> &bwd_computation_time);

  void getTransferTime(long num_epoch, std::vector<std::vector<float> > &fwd_transfer_time,
                       std::vector<std::vector<float> > &bwd_transfer_time);
  void stepTransferTime(int start_X, int start_y, std::vector<float> &fwd_transfer_time,
