...
void read_cifar10_file(Images &images, Labels &labels, const std::string &path,
                       std::size_t limit, Func func)
{
  ...
  {
    labels[start + i] = buffer[i * 3073+1];

    images.push_back(func());

    for (std::size_t j = 2; j < 3073; ++j)
    {
...
void read_test(const std::string &folder, std::size_t limit, Images &images,
               Labels &labels, Functor func)
{
  read_cifar10_file(images, labels, folder + "/test.bin", limit, func);
}
 ...
 */
template <typename Images, typename Labels, typename Functor>
void read_training(const std::string &folder, std::size_t limit, Images &images,
                   Labels &labels, Functor func)
{
  read_cifar10_file(images, labels, folder + "/train.bin", limit, func);
}
...
{
  read_test("cifar-100", limit, images, labels, func);
}
...
void read_training(std::size_t limit, Images &images, Labels &labels,
                   Functor func)
{
  read_training("cifar-100", limit, images, labels, func);
}
...
void read_cifar10_file_categorical(Images &images, Labels &labels,
                                   const std::string &path, std::size_t limit,
                                   size_t start)
{
 ...
 */
template <typename Images, typename Labels>
void read_training_categorical(const std::string &folder, std::size_t limit,
                               Images &images, Labels &labels)
{
  read_cifar10_file_categorical(images, labels, folder + "/data_batch_1.bin",
                                limit, 0);
  read_cifar10_file_categorical(images, labels, folder + "/data_batch_2.bin",
                                limit, 10000);
  read_cifar10_file_categorical(images, labels, folder + "/data_batch_3.bin",
                                limit, 20000);
  read_cifar10_file_categorical(images, labels, folder + "/data_batch_4.bin",
                                limit, 30000);
  read_cifar10_file_categorical(images, labels, folder + "/data_batch_5.bin",
                                limit, 40000);
}
 ...
 */
template <typename Images, typename Labels>
void read_test_categorical(const std::string &folder, std::size_t limit,
                           Images &images, Labels &labels)
{
  read_cifar10_file_categorical(images, labels, folder + "/test.bin", limit, 0);
}
 ...
 */
template <typename Images, typename Labels>
void read_training_categorical(std::size_t limit, Images &images,
                               Labels &labels)
{
  read_training_categorical("cifar-10/cifar-10-batches-bin", limit, images,
                            labels);
}
 ...
 * The dataset is assumed to be in a cifar-10 subfolder
 *
 * \param training_limit The maximum number of elements to read from data set
 * (0: no limit) \param test_limit The maximum number of elements to read from
 * test set (0: no limit)
 *
 * \return The dataset
 */
...
CIFAR10_dataset<Container, Image, Label> read_dataset_3d(
    std::size_t training_limit = 0, std::size_t test_limit = 0)
{
  CIFAR10_dataset<Container, Image, Label> dataset;

  read_training(training_limit, dataset.training_images,
                dataset.training_labels, [] { return Image(3, 32, 32); });
...
template <template <typename...> class Container, typename Image,
          typename Label = uint8_t>
CIFAR10_dataset<Container, Image, Label> read_dataset_direct(
    std::size_t training_limit = 0, std::size_t test_limit = 0)
{
  CIFAR10_dataset<Container, Image, Label> dataset;
...
CIFAR10_dataset<Container, Sub<Pixel>, Label> read_dataset(
    std::size_t training_limit = 0, std::size_t test_limit = 0)
{
  return read_dataset_direct<Container, Sub<Pixel>, Label>(training_limit,
                                                           test_limit);
}
